...
def sql_lex(kôd):
    ...
    for znak in iter(lex.čitaj, ''):
        if znak.isspace(): lex.token(E.PRAZNO)
            ...
            lex.pročitaj('-')
            lex.zvijezda(lambda znak: znak != '\n') #čitaj dok ne naletis na \n
            lex.pročitaj('\n')
            lex.token(SQL.KOMENTAR)
        elif znak.isalpha():
            lex.zvijezda(str.isalnum) # citaj dok nalazis na broj ili slovo
            #ključna_riječ(SQL, lex.sadržaj, False)
                #--> ako je SADRŽAJ gore u navedenim rijeci onda vrati tu kljucnu rijec inace False
